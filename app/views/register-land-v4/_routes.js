...
router.get("/register-land-v4/know-parcel-id", async function (req, res) {
  ...
  return res.render('register-land-v4/know-parcel-id', {
    value: value || '',
...
// router.get("/register-land-v4/confirm-land-parcel", async function (req, res) {
//   // Parcel service
...
//   res.render('register-land-v4/confirm-land-parcel', {
...
// router.post("/register-land-v4/confirm-land-parcel", function (req, res) {
...
router.post("/register-land-v4/confirm-land-parcel", function (req, res) {
  const confirmLandParcel = req.session.data['confirmLandParcel'];

  if (confirmLandParcel === 'no') {
    // User does not want this parcel, go back to know-parcel-id
...
router.post("/register-land-v4/estimate-land-parcel", async function (req, res) {
...
router.get('/register-land-v4/estimate-land-parcel-confirm', async function (req, res) {
  const parcelId = req.session.data['parcel-id'];

  // If we have a parcel ID, fetch additional data (land covers, hedgerows)
  if (parcelId) {
    try {
      // 1. Fetch parcel service data
      const url = `${process.env.PARCEL_SERVICE_URL}/${parcelId}`;
      ...
      return res.render('register-land-v4/estimate-land-parcel-confirm', {
        parcelId: parcelId,
        parcelBounds: req.session.data['parcel-bounds'],
      ...
      console.error('Error fetching parcel data in estimate-land-parcel-confirm:', error);
      // Fall through to render without additional data
    }
  }

  // Render without parcel data (no parcel found or error occurred)
  res.render('register-land-v4/estimate-land-parcel-confirm', {
    parcelId: req.session.data['parcel-id'],
    parcelBounds: req.session.data['parcel-bounds'],
    coords: req.session.data['estimate-coords'],
    zoom: req.session.data['estimate-zoom'],
...
// router.post('/register-land-v4/estimate-land-parcel-confirm', function (req, res) {
...
// router.post("/register-land-v4/upload-land-parcel-map", function (req, res) {
//   res.redirect("date-to-link-parcel-to-business");
// });
...
router.post("/register-land-v4/date-to-link-parcel-to-business", function (req, res) {
  res.redirect("check-parcel-details");
});

// Check parcel details
router.post("/register-land-v4/check-parcel-details", function (req, res) {
...
// Date to link parcel to business
router.post("/register-land-v4/declaration", function (req, res) {
  res.redirect("confirmation");
});

// Delete land parcel
// router.post("/register-land-v4/delete-parcel", function (req, res) {
//   res.redirect("check-parcel-details");
// });
...
router.get('/delete-parcel', function (req, res) {
  ...
  res.redirect('/register-land-v4/delete-parcel');
});
...
router.post('/register-land-v4/delete-parcel', function (req, res) {
  ...
  res.redirect('/register-land-v4/check-parcel-details'); 
});
